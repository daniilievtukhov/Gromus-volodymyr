...
import { Alert, Button, ButtonProps, Stack, createPolymorphicComponent } from "@mantine/core";
...
import { useHashtagsAnalytics } from "./personalizedHashtags/hooks/useHashtagsAnalytics";
...
export const HashtagDataPage = () => {
  ...
  const { isError, error } = useHashtagsAnalytics({
  ...
  if (isError) {
    if (isAxiosError(error) && error.response?.status === 400) {
      return (
        <Stack p={32} gap="md" bg="#0D0D0E" mih="100vh" align="center" justify="center">
          <Alert variant="light" color="0D0D0E" title="Hashtags not found" icon={<IconMoodSad />}>
  ...
  if (accountHashtagBalancedGroup.length > 0) {
    return (
      ...
      <FlexItems>
        <Stack align="center" justify="center">
          <Alert variant="light" color="orange" icon={<IconInfoCircle />} w={800}>
            We currently do not have data for your hashtag parameters. Try ask G another category or
            country. Or take a glance at your personalized hashtag page:
          </Alert>
