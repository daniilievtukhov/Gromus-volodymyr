...
export const ResetPassword = () => {
  ...
  const handleInputChange =
    (fieldName: keyof ResetPasswordData) => (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    var response = await axios
      .post(`${Links.api}/Account/ResetPassword`, {
        ...formData,
      })
      .then((response) => {
        localStorage.setItem("BEARER_TOKEN", response.data);
        navigate("/");
      })
      .catch(() => {
        ...
        <Center>
          <Stack gap={21} w={500} style={{ backgroundColor: "#2e2e2e", padding: "25px" }}>
            <Group justify="space-between">
              <CloseButton
                onClick={() => navigate(-1)}
                icon={<IconArrowLeft size={24} stroke={1.5} />}
              />
              <Image src={Logo} />
              <p></p>
            </Group>
            ...
            />
            <Button color="lime.4" c="black" type="submit">
              Change password
            </Button>
          </Stack>
        </Center>
