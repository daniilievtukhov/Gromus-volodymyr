...
export namespace ApiTranscriptionEdit {
    ...
    export const updateAIText = async (params: IRequest) => {
        return (await axios.post<IResponse>(`${linkAIUpdate}?${queryString.stringify(params)}`))
    }

    export const updateTranscriptionText = async (params: IRequest) => {
        return (await axios.post<IResponse>(`${linkTrascriptionUpdate}?${queryString.stringify(params)}`))
    }

    export const regenerateAIText = async (params: IRequest) => {
        return (await axios.post<IResponse>(`${linkRegenerateAIText}?${queryString.stringify(params)}`))
